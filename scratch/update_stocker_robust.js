...
// Let's use a simpler index search and replacement
...
if (startSettingsIdx !== -1 && endSettingsIdx !== -1) {
    ...
    console.log('Updated saveSettings block');
} else {
    console.error('saveSettings index not found!', startSettingsIdx, endSettingsIdx);
}

// 4. Update saveProfile
const startProfileIdx = content.indexOf('function saveProfile() {');
const endProfileIdx = content.indexOf('async function init() {');

if (startProfileIdx !== -1 && endProfileIdx !== -1) {
    const oldProfileBlock = content.slice(startProfileIdx, endProfileIdx);
    const newProfileBlock = `function saveProfile() {
            const stored = sessionStorage.getItem('stockerProfile');
            if (!stored) return;
            const p = JSON.parse(stored);
    ...
    console.log('Updated saveProfile block');
} else {
    console.error('saveProfile index not found!', startProfileIdx, endProfileIdx);
}

// 5. Update renderPredictionChart
const startPredIdx = content.indexOf('async function renderPredictionChart() {');
const endPredIdx = content.indexOf('if (predChartInstance) {');

if (startPredIdx !== -1 && endPredIdx !== -1) {
    const oldPredBlock = content.slice(startPredIdx, endPredIdx);
    const newPredBlock = `async function renderPredictionChart() {
            ...
            const labels = predictions.map(p => p.medicine_name);
            const data = predictions.map(p => p.predicted_demand);

            `;
    content = content.replace(oldPredBlock, newPredBlock);
    console.log('Updated renderPredictionChart block');
} else {
    console.error('renderPredictionChart index not found!', startPredIdx, endPredIdx);
}
...
console.log('Stocker.html robust update completed.');
